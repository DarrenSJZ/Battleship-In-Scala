...
@sfxml
class ShipPlacementP2Controller(private val gameGridP2: GridPane,
                                private val P2carrierButton: Button,
                                private val P2battleshipButton: Button,
                                private val P2cruiserButton: Button,
                                private val P2submarineButton: Button,
                                private val P2destroyerButton: Button,
                                private val P2endButton: Button) {
  ...
  // Ensure the player is set before using it in your methods
  def initialize(): Unit = {
    if (player2 == null) {
      throw new IllegalStateException("Player2 instance must be set before using the controller.")
    }
    ...
    P2endButton.setDisable(true) // Disable the end button until all ships are placed
...
//    println(player2.board.toString)
  }

  // Map to track ship buttons and their corresponding Ship objects
  private lazy val shipButtons = Map[Ship, Button](
    player2.ships(0) -> P2carrierButton,
    player2.ships(1) -> P2battleshipButton,
    player2.ships(2) -> P2cruiserButton,
    player2.ships(3) -> P2submarineButton,
    player2.ships(4) -> P2destroyerButton
  )

  // Initially disable the "End Turn" button
  P2endButton.setDisable(true)
  ...
  def P2carrierButtonHandler(): Unit = selectShip(player2.ships(0))
  def P2battleshipButtonHandler(): Unit = selectShip(player2.ships(1))
  def P2cruiserButtonHandler(): Unit = selectShip(player2.ships(2))
  def P2submarineButtonHandler(): Unit = selectShip(player2.ships(3))
  def P2destroyerButtonHandler(): Unit = selectShip(player2.ships(4))

  def P2EndButtonHandler(): Unit = MyApp.showP1AttackPhase(player1,player2)
  ...
  private def setupGridHandlers(): Unit = {
    ...
    val buttons = gameGridP2.children.asScala
  ...
  private def handleGridClick(x: Int, y: Int): Unit = {
    ...
    if (startX.isEmpty && startY.isEmpty) {
      ...
      if (length == currentShip.get.length) {
        val placementResult = player2.board.placeShip(startX.get, startY.get, currentShip.get, horizontal)
  ...
  private def checkAllShipsPlaced(): Unit = {
    ...
    P2endButton.setDisable(!allPlaced)
  }
  ...
  def updateView(): Unit = {
    if (startX.isDefined && startY.isDefined && endX.isDefined && endY.isDefined && currentShip.isDefined) {
      ...
      val background = new Rectangle {
        ...
        fill = Color.rgb(0, 0, 200, 0.5) // Semi-transparent gray
        stroke = Color.White // White outline
        strokeWidth = 1 // 1-pixel outline
      }
